let list uri : (string * string) list =
  ...
  in
  let|| dbs = (Mongoc.Client.get_database_names_with_opts client, "") in
  let u =
    List.concat_map
      (fun db_name ->
        let db = Mongoc.Client.get_database client db_name in
        let|| colls = (Mongoc.Database.get_collection_names_with_opts db, "") in
        let v =
          List.map
            (fun coll_name ->
...
let drop uri db_name coll_name =
  let ( let|| ) (r, err) f =
    ...
    let () = Mongoc.init () in
    let|| uri =
      (Mongoc.Uri.new_with_error uri, "Unrecognized URI")
    in
    let|| client =
      (Mongoc.Client.new_from_uri_with_error uri, "Failed to connect")
    in
