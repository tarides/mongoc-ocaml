...
module Error = struct
  ...
  let message (error : t) =
    ...
    |> CArray.to_list |> List.to_seq |> String.of_seq
end
...
let new_from_json ?len data : (t, Error.t) result =
  ...
  if is_null bson then Result.Error error else Result.Ok !@bson

let as_json ?length (bson : t) =
  let ptr = allocate t bson in
  let length =
    let some u = u |> Unsigned.Size_t.of_int |> allocate size_t in
    Option.fold ~none:(from_voidp size_t null) ~some length
  in
