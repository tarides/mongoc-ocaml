...
module Functions(F : Ctypes.FOREIGN) = struct
  ...
  module Uri = struct
    let new_with_error = foreign "mongoc_uri_new_with_error" (ptr (const char) @-> ptr Bson.Error.t @-> returning (ptr Types.Uri.t))
    let destroy = foreign "mongoc_uri_destroy" (ptr Types.Uri.t @-> returning void)
  end

  module Client = struct
    let new_ = foreign "mongoc_client_new" (ptr (const char) @-> returning (ptr Types.Client.t))
    let new_from_uri = foreign "mongoc_client_new_from_uri" (ptr (const Types.Uri.t) @-> returning (ptr Types.Client.t))
    let new_from_uri_with_error = foreign "mongoc_client_new_from_uri_with_error" (ptr (const Types.Uri.t) @-> ptr Bson.Error.t @-> returning (ptr Types.Client.t))
