let () =
  let ( let|| ) (r, err) f = match r with
    | Error s -> begin Printf.fprintf stderr "%s: %s\n" err (Mongoc.Bson.Error.message s); exit 1 end
    | Ok x -> f x in
  let () = Mongoc.init () in
  let|| uri = Mongoc.Uri.new_with_error "mongodb://127.0.0.1:27017", "Unrecognized URI" in
  let|| client = Mongoc.Client.new_from_uri_with_error uri, "Failed to connect" in
